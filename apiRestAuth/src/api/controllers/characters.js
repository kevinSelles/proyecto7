...
const getCharactersByCategory = async (req, res, next) => {
  try {
    ...
    if (!characters.length) {
      return res.status(404).json({ message: `No se encontraron personajes para la categoría '${category}'` });
    }
...
const putCharacter = async (req, res, next) => {
  try {
    const { id } = req.params;
    const characterUpdated = await Character.findByIdAndUpdate(id, req.body, {new: true,});

    if (!characterUpdated) return res.status(404).json("Personaje no encontrado");
